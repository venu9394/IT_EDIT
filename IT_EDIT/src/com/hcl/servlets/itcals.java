...
public class itcals extends javax.servlet.http.HttpServlet
{
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws javax.servlet.ServletException, java.io.IOException
  {
    ...
    eligibleQry.append(" AND FY IN ('"+financialyear+"')) GM,IFNULL(E.PAN,'') PAN");
   ...
   /* eligibleQry.append(" SELECT CASE WHEN MONTH(CURDATE())<=3 THEN CONCAT(YEAR(CURDATE())-1, '-',YEAR(CURDATE()))");
    ...
    eligibleQry.append(" WHERE EMPLOYEESEQUENCENO IN (" + id + ") ");
    // AND PAN.REFNO=(SELECT MAX(REFNO) FROM test.tbl_it_emp_details WHERE EMPLOYEEID IN (" + id + ") )");
    try
    {
